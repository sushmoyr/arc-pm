...
ARC is a local-first project manager. This repo is pinned to project **AP** (see `.arc/project.json`). Use ARC instead of inventing or remembering tasks.

## Mental model

```
Project AP
  └─ Tasks (id: AP-N, status, priority 1-5)
...
- **Surface dependencies you discover.** If `AP-5` can't ship until `AP-3` does, run `arc link AP-5 depends_on AP-3`.
